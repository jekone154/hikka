...
_RETRY_DELAY_SEC = 60  # задержка перед повтором при ошибке бэкапа
...
class HikkaBackupMod(loader.Module):
    ...
    async def set_backup_period(self, message: Message):
        """[time] | set your backup period in hours"""
        if (
    ...
    async def backupdb(self, message: Message):
        """| Backup database to Saved Messages"""
        txt = io.BytesIO(
            orjson.dumps(self._db, option=orjson.OPT_INDENT_2 | orjson.OPT_NON_STR_KEYS)
        )
    ...
    async def restoredb(self, message: Message):
        """| Reply to a db backup file to restore"""
        ...
        if re.search(r'"(heroku\.)(\S+\")', file.decode()):
            await utils.answer(
                ...
                reply_markup=[
                    {"text": "❌", "callback": self.convert, "args": ("n", file.decode())},
                    {"text": "✅", "callback": self.convert, "args": ("y", file.decode())},
                ],
            )
            return

        _current_token = self._db.get("hikka.inline", "bot_token", None)
        ...
        if _current_token:
            self._db.set("hikka.inline", "bot_token", _current_token)
    ...
    async def restoremods(self, message: Message):
        """| Reply to a mods backup to restore"""
        if not (reply := await message.get_reply_message()) or not reply.media:
            await utils.answer(message, self.strings("reply_to_file"))
            return
                ...
                with zipfile.ZipFile(file) as zf:
                    with zf.open("db_mods.json", "r") as modules:
                        ...
                        if isinstance(db_mods, dict) and all(
                            isinstance(k, str) and isinstance(v, str) and utils.check_url(v)
                            for k, v in db_mods.items()
                        ):
    ...
    async def backupall(self, message: Message):
        """| Backup database + modules in one file"""
        db = io.BytesIO(
            orjson.dumps(self._db, option=orjson.OPT_INDENT_2 | orjson.OPT_NON_STR_KEYS)
        )
        db.name = "db.json"
    ...
    async def restoreall(self, message: Message):
        """| Reply to a .backup file to restore everything"""
        if not (reply := await message.get_reply_message()) or not reply.media:
            await utils.answer(message, self.strings("reply_to_file"))
            return
    ...
    def _convert(self, backup):
        fixed = re.sub(r"(heroku\.)(\S+\")", lambda m: "hikka." + m.group(2), backup)
